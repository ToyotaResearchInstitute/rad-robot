...
-- Not as efficient as multiple programs
...
local function log_announce(obj, channel)
  if not log then return false, "Cannot serialize" end
  local str = log:write(obj, channel)
  if not str then return false, "Could not log data" end
  if skt_mcl then skt_mcl:send(str) end
  return #str
end
...
local function real_sensors()
  ...
  while true do
    ...
    elseif js_reading then
      ...
      log_announce(js_reading, 'joystick')
...
local function entry()
  ...
  if has_skt and not flags.mcl==0 then
    local err
  ...
  if has_hokuyo then
    ...
    if skt_hokuyo then
      ...
      log_announce(hokuyo_params, 'hokuyo_params')
