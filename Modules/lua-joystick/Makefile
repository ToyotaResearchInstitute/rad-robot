...
# NOTE: Only Linux has a joystick API
ifndef OSTYPE
...
$(TARGET): $(OBJS)
	$(LD) $(LIBFLAG) $(shell pkg-config libusb-1.0 --libs) -lpthread -o $@ -L$(LUA_LIBDIR) $(OBJS)

%.o: %.c
	$(CC) -c -o $@ $< -I$(LUA_INCDIR) $(CFLAGS) $(shell pkg-config libusb-1.0 --cflags)
