...
function matrix.flatten( mtx )
	local flat = {}
	local n_rows = #mtx
	local n_cols = #mtx[1]
	for i_row = 1,n_rows do
		for i_col = 1,n_cols do
			table.insert(flat, mtx[i_row][i_col])
		end
	end
	return flat, {n_rows, n_cols}
end

function matrix.diag(inp)
	local n = #inp
	-- Make diagonal matrix from vector
	if type(inp[1])=='number' then
		local mat = matrix:new(n, n)
		for i=1,n do mat[i][i] = inp[i] end
		return mat
	else
		-- Make vector from matrix diagonal
		assert(n==#inp[1], "Not square")
		local vec = {}
		for i=1,n do vec[i] = inp[i][i] end
		return vec
	end
end
