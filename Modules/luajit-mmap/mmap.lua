...
local function close(tbl)
  local ptr, sz = unpack(tbl)
...
function lib.open(filename)
  ...
  if ptr == nil then
    C.perror("mmap")
    return false, "Could not mmap file"
  end
  return setmetatable({ptr, sz}, mt)
end
...
function lib.shm_open(filename, sz)
  local fd = has_rt and rt.shm_open(filename, O_CREATE_RW, S_IRWUSR)
             or C.shm_open(filename, O_CREATE_RW, S_IRWUSR)
  if fd < 0 then
