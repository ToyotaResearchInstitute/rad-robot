...
def parse_lcm_hdr(lcm_hdr):
    ...
    assert lcm_magic == LCM_MAGIC, "Bad LCM magic [{}]".format(
        binascii.hexlify(lcm_hdr)
    )
...
class LMPReader:
    ...
    def __init__(self, fname_log, *kargs, fname_idx=None, channel="harvest", **kwargs):
        """
        ...
        # Generate the index if it does not exist, else read the existing index
        if os.path.isfile(fname_idx):
            self.read_index(self, fname_idx)
        else:
    ...
    def read_index(self, fname_idx, use_mmap=False):
        ...
        if use_mmap:
            raise NotImplementedError(
                "Memory mapping of the log file is not supported, yet"
            )
        ...
        assert len(self.sizes) == self.n_entries
        assert len(self.timestamps) == self.n_entries

    def index(self, fname_idx=None):
        """
        (Re)index the offsets, sizes and timestamps
        """
        # Set the indices
        self.offsets = []
        self.sizes = []
        self.timestamps = []
        # Check if writing the index to disk
        if isinstance(fname_idx, str) and os.access(fname_idx, os.W_OK):
            f_idx = open(fname_idx, "ab")
        else:
            f_idx = None
        # Rewind the log
        self.f_log.seek(0)
        # Iterate through the log, via seeking
        while True:
            ...
            sz_entry = LCM_HDR_SZ + sz_data
            # Add to our index information
            self.offsets.append(offset)
            self.sizes.append(sz_entry)
            self.timestamps.append(t_us)
            # Move the cursor within the log to the next entry
            self.f_log.seek(sz_data, io.SEEK_CUR)
            # Save the index to disk
            if f_idx is not None:
                idx_entry = struct.pack(IDX_STRUCT_FMT, offset, sz_entry, t_us)
        ...
        # Update the index size and the number of elements
        self.idx_sz = self.n_entries * IDX_STRUCT_SZ
        self.n_entries = len(self.timestamps)

    def get_slice(self, idx_slice):
        """
        Grab information at a slice
        ...
        it_raw = map(
            os.pread,
            repeat(self.fd_log),
            self.sizes[idx_slice],
            self.offsets[idx_slice],
        )
    ...
    def __copy__():
        raise NotImplementedError("Need an effiient copy mechanism")
        ...
        for i, (t_a, t_b) in enumerate(intervals):
            ...
            if index_only:
                ...
                with open(os.path.join(dest, fname_index), "wb") as f_idx:
                    for off, sz, ts in zip(
                        offsets_interval, sizes_interval, timestamps_interval
                    ):
                ...
                subprocess.run(
                    [
                        "dd",
                        "if={}".format(self.fname_log),
                        "of={}".format(os.path.join(dest, fname_outlog)),
                        "skip={:d}".format(offset_a),
                        "count={:d}".format(count_bytes),
...
class LMPTransformed(LMPReader):
    ...
    def __init__(
        self, *kargs, output_transform=None, sequence_transform=None, **kwargs
    ):
        super().__init__(*kargs, **kwargs)
        self.output_transform = output_transform if callable(output_transform) else None
        self.sequence_transform = (
            sequence_transform if callable(sequence_transform) else None
        )
...
if __name__ == "__main__":
    ...
    if args["split"]:
        ...
        with open(args["split"], "r", newline="") as tsvfile:
            trialsreader = csv.reader(
                tsvfile, delimiter="\t", quoting=csv.QUOTE_NONNUMERIC
            )
            log_intervals = list(trialsreader)
        d.split(log_intervals)
